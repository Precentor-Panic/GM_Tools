...
Body — the **Option A front-matter band** (2026-09-02, from the design-canvas Prep-header round; replaced the old seven-block stack):

1. **Title row** — mono uppercase place label (teal, 10px) INLINE before the scene name (`contenteditable`, Spectral).
2. **Intent row** — the objective (`contenteditable`, 14px) plus the `intent-run-toggle` chip: "shown in Run" ↔ "GM only · off in Run" (persists `scene.objectiveInRun` through the ordinary patch route; absent/true = the Run spread renders the Objective box, false = it doesn't — the intent is often just the GM's scene-locator note).
3. **Chip row** — the Stage-for-Foundry toggle (+ its quiet status line) and the always-visible map chip, side by side.
4. **About the place** — a `<details>` disclosure (`scene-about-place`): summary = mono label + one-line description preview; OPEN by default only while the place has no description (amber-tinted summary then — develop matters most exactly then). Inside, with no dividers: the description grid (edits write the *graph node*), the missing-description banner when empty, and the "✦ develop this place" ghost.
5. **Read aloud** — labeled "READ ALOUD — OPENS RUN": the ONE home for the opener (later read-alouds are beat elements; the old duplicate came from the retired seed skeleton). Ghost "✦ Draft this…" as before.
6. **Elements** — rows immediately; `✦ propose elements here` moved to the BOTTOM beside the other adders (on an empty scene the empty list puts it visually at the top anyway).
...
**Seed run skeleton** (Prep ghost RETIRED 2026-09-02 — Option A band; Russell: new elements default sensibly, sort them in the lanes): `POST …/run-layout/seed` and `wf_seed_run_skeleton` remain for agents/tests, creating placeholder elements per `scene.kind` (narrative: read-aloud, 3 dressing, 2 beats, GM note, exits; combat adds Enemies + Sketch; transit: read, beat, exits). Placeholders (`run.placeholder`) render dashed with a `· fill me` suffix in Prep, are hidden in Run while empty, and drop the flag on their first real edit. Idempotent by role.
...
**Variants — tabs on the card** (Treatment B, adjudicated 2026-09-01 via the design-canvas round). Inside a group card or the GM-notes fold, the variant-carrying members' names render as a clickable **tab row** (mono uppercase, active = ink + rust underline; the active state's own section heading is suppressed — the tab IS its heading; variant-less members always show with headings). **Clicking a tab is a LOCAL flip only** — nothing is written; variants turn the page on *data*, while a real change of what the table sees is a scene switch (the future Foundry push/unstage/activate pass). `scene.activeVariants` stays the persistent, MCP-visible switch and SEEDS the tabs (a listed variant's tab starts active; else the FIRST tab — the adjudicated default); the GM's local flips override the seed, survive live-refresh rebuilds (an MCP burn landing mid-session never resets your tabs) and Prep↔Run trips, and clear when a different scene renders. Gated gm boxes COUNT toward the ≥2 fold (they become tabs). LOOSE variant elements (ungrouped, not in the fold) keep the old plain gating: empty activeVariants = visible, gated = dropped. The rules live in `planRunSpread` (`run-layout.mjs`, v4 — gating is a `variantHidden` STAMP reaching the planner, no longer a pre-filter).

**Reveal-seeded tabs** (narrative-state round, v4). A state marked `revealTab` in the run-chip popover ("Seed tab on reveal", enabled only with a variant) becomes the card's SEEDED active tab whenever its bound graph entity — the element's own `graphEntityId`, else the group's first-bound member's — has narrative-state `revealed`. Seed precedence: **local pick → revealTab-when-revealed → activeVariants → first tab.** This is the mid-session reveal wire: an NPC card carries a "Cover" state and a "Revealed" state; when the table learns the truth (a wrap apply, or `wf_set_reveal_state` live), the reveal joins the run-version fingerprint, the 3s poll rebuilds, and the card flips its default state at the table — while a local tab click still overrides for this table, this session.

**Session wrap-up** (Plan page, narrative-state round). A collapsed panel on the plan detail: the withheld roster (every narrative-state record ≠ revealed), agent-suggested transitions from the plan's session notes (GM edits/deselects; the LLM only ever suggests — nothing applies without the explicit Apply, which stamps `source:"wrap"` + sessionNumber into each record's permanent transition history), and an auto-generated player-facing "truth notes" markdown recap built from ONLY the just-revealed records (with a copy-for-players button). Scene-level "Wrap this scene" (graph proposals from notes + element promotion) is unchanged and separate — the plan-level panel is about what the TABLE now knows.
