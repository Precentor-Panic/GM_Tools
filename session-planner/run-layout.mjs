 ...
 * imported by the store (session-planner/scene-elements.mjs, to infer
 * layouts server-side), by the HTTP server, AND by the browser (served at
 * `/shared/run-layout.mjs` by review-ui/server.mjs) so the inference
 * fallback is defined exactly once. Keep it that way — no imports.
 *
