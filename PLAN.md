...
| 5 | Import-from-writeup | Yes | **Ready — see `plans/phase-5-tasks.md`.** Grounded in a confirmed fact: `interchange.mjs`'s `importGraph` already does name-based dedup, stub-entity creation, and merge — pure, tested, reusable as-is. This phase is genuinely narrow: an LLM call turning freeform text into a WFI-shaped proposal (task 5.1), a dry-run through the existing merge logic converted into a normal review batch (task 5.2, plus a new additive `SourceKind` value for auditability), and the MCP tool + existing commit path (task 5.3) — not a new merge algorithm. |
