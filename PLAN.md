...
| 8 | Rubber-duck creative mode | Yes | **Ready to execute** — design round complete: a working-GM persona gave concrete requirements (the "ask" moment sits after a first pass, not before submission; first reaction is three cheap one-sentence interpretive framings, not full competing proposals; plain rejects should prompt a quick-pick reason, opt-in only; standing global setting, not per-import), and a creative-collaboration/interaction-design persona translated these into a decisive technical spec — see `plans/phase-8-review.md`. Task plan at `plans/phase-8-tasks.md`: `proposeFramingsFromWriteup` (new, own prompt file, deliberately cheap/fast model tier) sits in front of Phase 5's existing `writeup-import.mjs` pipeline unchanged; the one open design question (does a plain reject jump to a new full proposal or loop back to new framings?) was resolved decisively — loops back to framings, bounded to one re-framing round, with explicit-note rejects always skipping the loop entirely via the existing `regenerateWriteupImport` fast path; new `mutation-engine/user-settings.mjs` (global, `human-review.mjs`'s established convention) read once at submission and snapshotted onto `batch.scope.rubberDuck` so a mid-review toggle flip can't retroactively change an in-flight batch; new review-ui framing-selection screen (flagged explicitly as new frontend scope, not a footnote to Phase 6's build). |
