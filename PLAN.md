...
| 7 | Foundry-optional visual graph editor — rescoped per direct user feedback (see Phase 10's context): not just an occasional "explore the graph" utility, but a persistent, switchable-with-the-list view that color-codes nodes by deferred-debt/unreviewed status, directly addressing the Batch Review list's "overwhelming" feeling for large writeup-import batches. Queue (list) stays the default landing view; the graph view is one click away — both preserved, neither replacing the other. | Yes | Deferred — needs its own DM/interaction-design review round before a task plan is drafted, same discipline as every prior UI phase. |
...
| 11 | **Per-node content generation** (rubber-duck mode for entity content: description, actions, secrets, potential rolls — the generalized version of the old, deprioritized "Phase 0" NPC/location-card idea) | Yes | Flagged, not yet scoped — needs its own design round before a task plan is drafted. Explicitly kept separate from Phase 7's graph-view work per the project owner's own call. |
