...
| 11 | **Per-node content generation** (rubber-duck mode for entity content: description, actions, secrets, potential rolls, type-specific templates per person/place/faction/object/event — the generalized version of the old, deprioritized "Phase 0" NPC/location-card idea) | Yes | Design round complete — DM + creative-collaboration review produced a concrete spec: framing-first (same discipline as whole-writeup rubber-duck mode, one level down), triggered post-commit not in Batch Review, a new separate `PrepDoc`-style store (not the mutation-engine pipeline — prep content isn't a graph-structure diff), field-granular living-doc regeneration after initial accept, no bulk "develop all." See `plans/phase-11-review.md`. **One open question before `plans/phase-11-tasks.md` can be drafted**: whether/how secret-field content is excluded from `wf_sync_to_foundry` to avoid a GM-only secret leaking into a player-visible Foundry journal entry — a real privacy decision, not yet resolved. |
