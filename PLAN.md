...
| 10 | **Per-entity narration & persistence** — replaces whole-batch narration with per-entity narration, persisted durably (a new GM_Tools-side store, not a World Fabric schema change) and targeted using the entity's own real graph connections. Directly fixes a cluster of real bugs found in first hands-on use: identical narration across every row in a batch, "regenerate" recycling the same generic text regardless of which row it was clicked from, no memory of a previously-accepted narration across page loads, and a default "session zero"-style framing caused by narrating with zero grounding context (review-ui never passed a location/reachable-areas hint at all). See `plans/phase-10-review.md` (root cause + decisions, grounded directly in the project owner's own usage feedback rather than a persona review) and `plans/phase-10-tasks.md` (task plan). | Yes | Ready to execute. |
