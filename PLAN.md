...
| 6 | Dedicated web review UI | Yes | **Ready — see `plans/phase-6-tasks.md`.** No longer deferred: this is now the priority, the actual visible surface for everything Phases 1-5 built. Design gathered from two independent perspectives before building anything — a working-GM persona's stated day-to-day needs, then an HMI/UI-design persona's concrete interaction design grounded in those needs (full record in `plans/phase-6-review.md`). Confirmed before writing the task file: every library function this UI needs already exists and is reusable as-is (review-state, grain, narrate, rollback, pending-ledger, human-review) — this phase is a thin HTTP layer + the designed frontend, not new backend logic. |
