...
| 7 | Foundry-optional visual graph view — rescoped per direct user feedback (see Phase 10's context): not just an occasional "explore the graph" utility, but two connected surfaces — a List/Graph toggle on the Batch Review screen (context-scoped: batch entities + one-hop persisted neighbors) and a standalone whole-graph nav view (filtered to unreviewed/deferred-debt entities by default) — directly addressing the Batch Review list's "overwhelming" feeling for large writeup-import batches while color-coding status. Queue (list) stays the default landing view. | Yes | **Ready to execute, queued behind Phase 10** (concurrent file edits would conflict). Design round complete — DM + interaction-design review produced a concrete node/interaction spec (circle nodes sized by degree, entity-type fill, independent unreviewed-border/deferred-debt-badge channels, shared accept/reject selection state between list and graph, multi-select feeding the existing bulk action bar) — see `plans/phase-7-review.md` and `plans/phase-7-tasks.md`. |
...
| 11 | **Per-node content generation** (rubber-duck mode for entity content: description, actions, secrets, potential rolls, type-specific templates per person/place/faction/object/event — the generalized version of the old, deprioritized "Phase 0" NPC/location-card idea) | Yes | Design round complete — DM + creative-collaboration review produced a concrete spec: framing-first (same discipline as whole-writeup rubber-duck mode, one level down), triggered post-commit not in Batch Review, a new separate `PrepDoc`-style store (not the mutation-engine pipeline — prep content isn't a graph-structure diff), field-granular living-doc regeneration after initial accept, no bulk "develop all." **Ready to execute, queued behind Phases 10 and 7** (concurrent file edits would conflict). The open privacy question is resolved: PrepContent (including secret fields) **never syncs to Foundry** — purely GM-side reference material, no player-visible-journal risk. See `plans/phase-11-review.md` and `plans/phase-11-tasks.md`. |
