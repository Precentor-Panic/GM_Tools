...
| 4 | Rollback hardening + unreviewed-accumulation tracking | Yes | **Ready — see `plans/phase-4-tasks.md`.** Grounded in a confirmed, still-open gap: `rollback.mjs`'s own doc comment flags newly-created entities as unrollback-able pending Phase 2b's id-read-back — Phase 2b shipped without ever adding it, confirmed by reading `headless-apply.mjs` directly. Task 4.1 closes that. Task 4.2 builds unreviewed-accumulation tracking (`lastHumanReviewedAt`, distinct from batch-accept-all) from scratch — nothing exists yet beyond the original planning mention. |
