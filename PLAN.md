...
| 3 | Live on-demand diff mode | Yes | **Next up — see `plans/phase-3-tasks.md`.** Grounded against real code before writing: `time-skip/scope.mjs`'s `seed` mode (single `anchorId`) already *is* the live-resolution mechanism — nothing new to build there. What's actually missing: the scene-narration LLM call from the project's original two-call pattern (mutation call + narration call), never built in any phase so far, plus real latency measurement for live/synchronous use. |
| 3.5 | **Deferred/lazy consequence resolution** (informal name) — for large time-skips, resolve only a per-cycle "headline" scope eagerly (reusing Phase 3's single-seed mechanism) and record everything else as a cheap, unresolved tag in a new pending-resolution ledger. Zero further cost unless a tagged entity is later explicitly asked about. Design is fully settled (ledger = per-entity file under `pending-resolution/<world>/`, reusing `review-state.mjs`'s locking; explicit-trigger-only, default resolve scope = requested entity + up to its top-K impact-scored 1-hop neighbors, capped by count not sub-clustering to avoid hub-node fan-out cost; multi-cause entries rendered chronologically, hydrated from the source batch at resolve time, not duplicated into the ledger) — see conversation history / a future `plans/phase-3.5-tasks.md`. **Task file deliberately not written yet** — drafting it before Phase 3's narration call actually exists risks the same staleness Phase 2's original plan hit; write it once Phase 3 lands and there's real code to ground it in. | Yes | Design settled, depends on Phase 3, task plan deferred on purpose |
