...
## 12. Phasing decision (PM pass, adopted)

A dedicated PM-persona pass read this record plus the current `scenes.mjs`/`manual-undo.mjs`/`prep-content-ops.mjs` and produced the following, adopted as-is:

**§10's open questions, resolved:**
- **Scene-to-scene linkage**: derived from World Fabric graph adjacency on demand, never stored on the scene record. Chain order is a provable fact of the graph (§1's own rule — order follows real location adjacency, arbitrary reordering isn't a requirement), so a stored pointer would just be a stale-able cache of something already computable for free via `graph.mjs`/`corridor.mjs`'s existing, already-tested traversal functions. `parentSceneId` (fork/branch lineage — a different axis, time not space) stays stored as-is, unaffected.
- **Scene-local rollback**: needs a genuine scene-scoped action history, not `manual-undo.mjs`'s existing single global slot. That module is explicitly last-write-wins with no history list (its own header comment says so) — the moment "Develop this scene" touches a second node, the single slot overwrites the first node's undo, directly failing the "quickly unwind whatever I just did in this scene" requirement. New, small module, reusing `manual-undo.mjs`'s `UndoAction` shape but as an ordered list scoped to one scene-development session, consumed as a grouped reverse-order undo. Single-node "Develop this node" is unaffected, keeps using the existing global slot.
- **Transit/path entity type**: NOT a new entity-type enum value. The `type` enum is independently duplicated (not shared from one source) across at least four files (`wf-mcp-server/index.mjs`'s mutation-apply validation, `scan-mentions.mjs`, `writeup-import.mjs`, `prep-content.mjs`) — adding a new value risks a coordinated multi-file change landing as a partial fix, with sibling paths (fuzzy-dedup, prep-content generation, UI type-filtering) silently mishandling the new type later, the exact recurring bug shape this project has hit before. Transit entities commit as `type: "place"` with a distinguishing attribute instead — already permitted by §3's own language ("stay exactly as generic as 'Path'... indefinitely").

**Two engine-layer requirements identified that the original design record implied but didn't spell out, both belong in the engine phase, not the UI phase:**
- A **server-side batch development orchestrator** for "Develop this scene" — running the existing propose→reframe→generate→accept round-trip across every scene member with real sequencing and partial-failure handling. Must not be a client-side loop calling the single-node flow N times (no natural place to handle node 3-of-6 erroring mid-batch; same bug shape as an earlier phase's client-side-loop-hits-LLM-unconditionally issue).
- A **fast, single-shot ad-hoc generation primitive** for the mid-session "+" flow (§7) — genuinely distinct from the multi-round Q&A pattern, since "one field, one button, fast" rules out reusing the full reframe-round machinery.

**Final phase sequence:**
1. **Phase 22 — Scene engine** (no UI): linkage-derivation query, transit-entity creation (`type:"place"` + attribute), arbitrary-node-add + reachability-aware intervening-node offer (§4 — independently buildable/testable first within this phase, no dependency on the rest), the new scene-scoped batch-undo module, the server-side batch "develop this scene" orchestrator, the fast quick-gen primitive, and routes for all of it.
2. **Phase 23 — Scene construction UI**: depends fully on Phase 22 (matches this project's established engine→UI precedent). Chain display, add/remove-node UI, Develop-node/Develop-scene UI (per-node review of the batch flow), the scene-local rollback control, equal-weight Add Event/Add Encounter, mid-session "+" ad-hoc scene creation.
3. **Phase 24 — Scenes tab**: depends only on Phase 22, NOT on Phase 23 — a sibling consumer of the engine, can be built in parallel with Phase 23 rather than queued behind it. Cross-scene browse/search UI over Phase 22's linkage query.
4. **Not yet a phase — information-cockpit design/research round** (§8): deliberately pulled out of the numbered sequence. Needs its own `phase-N-review.md` grounded in real cockpit/dashboard UI prior art AND in Phase 23's actually-shipped output (researching against a UI that doesn't exist yet would repeat the exact mistake that produced Phase 17's under-informative cards) — sequenced after Phase 23 ships, independent of Phase 24. Do not scope this into engine+UI phases until that design round exists.
